    ...
    async (uint8: Uint8Array) => {
      ...
      try {
        ...
        if (result?.success) {
          ...
          toast.error(`Failed to process the PDF file: ${reason}`);
        }
      } catch (err: any) {
  ...
  const onGenerateProof = async () => {
    ...
    try {
      ...
      const makeProofRequest = (sub_string: string) =>
        generateProof({ pdf_bytes, page_number: 1, offset: 0, sub_string });
  ...
  const onVerifySnarkProof = async () => {
    ...
    try {
      ...
      const verifyProof = async (proof: any) => {
        ...
        const result: any = await verifyProofOnProver(proof);

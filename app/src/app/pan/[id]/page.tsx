...
export default function EnhancedPANVerifier({
  ...
  const fetchVerificationData = async () => {
    setLoading(true);
    try {
      const data = await getVerifyPan(id, accessToken);
      if (!data.success) {
  ...
  const resetPDFState = useCallback(() => {
    ...
    setPdfBytes(null);
    setPages([]);
    setStatus("No document selected yet");
    setProofData(null);
  ...
  // Receives bytes already read and magic-byte checked by <PdfDropzone />.
    ...
    async (uint8: Uint8Array) => {
      ...
      try {
        ...
        if (result?.success) {
          ...
          const reason = result?.error || "The PDF could not be parsed";
          setStatus(`PDF processing failed: ${reason}`);
          toast.error(`PDF processing failed: ${reason}`);
        }
      } catch (err: any) {
        const reason = err?.message || String(err);
        setStatus(`Error processing file: ${reason}`);
        toast.error(`Error processing file: ${reason}`);
      } finally {
        setProcessing(false);
      }
    },
  ...
  const onGenerateProof = async () => {
    ...
    try {
      const pdf_bytes = Array.from(pdfBytes);

      const nameProofData = await generateProof({
        pdf_bytes,
        page_number: 1,
        offset: 0,
      ...
      const panProofData = await generateProof({
        pdf_bytes,
        page_number: 1,
        offset: 0,
  ...
  const onVerifySnarkProof = async () => {
    ...
    try {
      ...
      if (proofToVerify.nameProof) {
        const nameResult: any = await verifyProof(proofToVerify.nameProof);
        nameVerificationResult = nameResult.success || nameResult.valid;
      }

      if (proofToVerify.panProof) {
        const panResult: any = await verifyProof(proofToVerify.panProof);
            ...
            <div className="space-y-6">
              <Card className="shadow-lg hover:shadow-xl transition-all duration-200 border-0 bg-white/70 dark:bg-slate-900/70 backdrop-blur-sm">
                ...
                <CardContent className="space-y-6">
                  <PdfDropzone
                    onFileAccepted={(bytes) => processFile(bytes)}
                    onCleared={resetPDFState}

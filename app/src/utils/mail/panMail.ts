...
export const sendNamePanVerificationEmail = async (
  ...
  try {
    ...
    const accessToken = createAccessToken("pan", String(id), to);
    const verifyUrl = `${process.env.API_URL}/pan/${id}?t=${encodeURIComponent(
      accessToken
    )}`;
    ...
    const mailOptions = {
      ...
      to,
      subject: `Verify Name & PAN ID Request - ZeroVerify`,
      html: `
        <div style="
...
export const sendConfirmedPanVerificationMail = async (
  ...
  try {
    ...
    const mailOptions = {
      from: `"ZeroVerify" <${process.env.MAIL_USER}>`,
      to,
      subject: `Name & PAN ID Verification Confirmed - ZeroVerify`,
      html: `
        <div style="
          font-family: Arial, Helvetica, sans-serif;
          background-color: #ffffff;

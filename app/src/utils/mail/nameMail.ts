...
export const sendNameVerificationEmail = async (
  ...
  try {
    ...
    const accessToken = createAccessToken("name", String(id), to);
    const verifyUrl = `${process.env.API_URL}/name/${id}?t=${encodeURIComponent(
      accessToken
    )}`;
...
export const sendConfirmedVerificationEmail = async (
  ...
  try {
    ...
    const mailOptions = {
      from: `"ZeroVerify" <${process.env.MAIL_USER}>`,
      to,
      subject: `Name Verification Confirmed - ZeroVerify`,
      html: `
        <div style="
          font-family: Arial, Helvetica, sans-serif;
          background-color: #ffffff;
